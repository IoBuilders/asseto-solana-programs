...
| `["asset_class_version", config_id, version]` | `factory` | Per-version `AssetClassVersion` PDA — **zero-copy**, fixed-capacity `[u8; FUNCTIONALITIES_BYTES_MASK]` functionality bit-mask + state; created `Draft` by `init_asset_class_version` (copies the previous version's mask as the starting point), bits turned on in chunks by `write_asset_class_version_mask` (OR-only → append-only, can't clear inherited bits), sealed `Ready` by `finalize_asset_class_version` |
