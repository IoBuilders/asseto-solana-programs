...
- `roles` — flat append-only `u16` role ids (currently `ROLE_ADMIN = 0` … `ROLE_DEACTIVATE = 8`) + `ROLES_MASK_OFFSET`; mirror of `functionalities` for `access-control`. A unit test asserts ids are sequential from 0.
