...
| `["hold_position", mint, token_account]` | `hold` | Per-`(mint, token_account)` `HoldPosition` — `held_amount` (the lien the transfer hook subtracts from the spendable balance) + `next_hold_id` (monotonic, never reset). Created by the account's first `create_hold` and never closed |
