...
The Token-2022 metadata extension (managed by `cmtat-metadata-update`) keeps the rest of the bond term sheet — `currency`, `unique_identifier`, LEIs, schedule details, etc. Only the fields a consuming program needs to reason about programmatically live here.
...
`cmtat-bond` exposes **no** read instruction. Consumers load the PDA themselves — same data, no CPI cost, no extra IDL surface to maintain.
...
```rust
use cmtat_bond::state::BondTerms;
...
The `Account<'info, BondTerms>` constraint enforces (1) the address derived from the seeds, (2) ownership by `cmtat-bond`, (3) the Anchor discriminator, and then Borsh-deserialises the typed struct. Reading individual fields is then ordinary Rust struct access — no allocations, no scanning, no string parsing.
