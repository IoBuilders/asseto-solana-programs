...
3. CPI → `cmtat_freeze::unblock_account(token_account)` signed with `["permanent_delegate", mint, bump]`
4. `invoke_signed` → `burn(token_account, mint, operations_authority, amount)` signed with `["permanent_delegate", mint, bump]`
5. CPI → `cmtat_freeze::block_account(token_account)` signed with `["permanent_delegate", mint, bump]`
