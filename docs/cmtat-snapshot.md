# cmtat-snapshot — Program Reference
...
Snapshots are taken exclusively by `cmtat-coupon::create_coupon`, which CPIs `take_snapshot` signed by its `coupon_authority` PDA. Three other callers append entries to the running histories whenever they move tokens (so long as a snapshot is active): `mint_authority` (cmtat-mint), `permanent_delegate` (cmtat-operations), and `transfer_hook_authority` (cmtat-transfer-hook).
...
`calling_authority` must be the `coupon_authority` PDA owned by `cmtat-coupon` (seeds: `["coupon_authority", mint]`). Only `cmtat-coupon::create_coupon` can produce that signature via `invoke_signed`, so every snapshot in the workspace is anchored to a coupon.

Pause / deactivate / deployer checks live in `cmtat-coupon::create_coupon` — `take_snapshot` itself trusts its caller, matching the style of the other auxiliaries (`update_totalsupply_snapshot`, `update_holderbalance_snapshot`).

### Accounts

| Account | Mut | Signer | Type | Notes |
|---|---|---|---|---|
| `calling_authority` | no | yes | Signer | `coupon_authority` PDA owned by `cmtat-coupon` |
...
- `["mint_authority", mint]` owned by `cmtat-mint`, or
...
- `["mint_authority", mint]` (cmtat-mint)
- `["permanent_delegate", mint]` (cmtat-operations)
- `["transfer_hook_authority", mint]` (cmtat-transfer-hook)
...
All program IDs are hardcoded — the CPI callers (`cmtat-mint`, `cmtat-operations`, `cmtat-transfer-hook`, `cmtat-coupon`) depend on `cmtat-snapshot`, so importing any of them here would create a cycle.
...
pub const CMTAT_TRANSFER_HOOK_PROGRAM_ID: Pubkey = Pubkey::new_from_array([...]);
pub const CMTAT_COUPON_PROGRAM_ID:        Pubkey = Pubkey::new_from_array([...]);
```
