# cmtat-treasury — Program Reference

Program ID: `CBxS9txE8qZqZkNXhTaWE42Ur3J3GtYv1ufLfNDNUEct`

Pays coupon interest to bond holders in a separate token mint (the *payment mint*, e.g. a stablecoin) — distinct from the bond mint the rest of the workspace targets. The payment mint may be **classic SPL Token or Token-2022** — `cmtat-treasury` uses Anchor's token *interface* so either is accepted.

Both instructions are **management** — only callable by the deployer recorded in `cmtat-deploy`'s `mint_owner_pda`, and only while the bond mint is neither paused nor deactivated.
...
- `interest_rate`, `interest_dec`, `par_value`, `par_value_dec`, `issuance_date`, `day_count_convention` come from `bond_terms` (cmtat-bond, seeds `["bond_terms", mint]`).
...
- `coupon.snapshot_id`, `coupon.period_start_date`, `coupon.period_end_date`, and `coupon.payment_date` come from `coupon` (cmtat-coupon, seeds `["coupon", mint, coupon_id]`).
- `holder_balance` is read by **CPI to `cmtat-snapshot::get_holderbalance_snapshot_at(coupon.snapshot_id)`** — the snapshot program handles the case where the holder hasn't transacted post-snapshot by falling back to the live token-account balance.
- `elapsed_seconds = coupon.period_end_date − coupon.period_start_date` — the accrual window of *this* coupon, not the time since bond issuance. Each coupon accrues independently. The non-positive case can't normally happen because `cmtat-coupon::create_coupon` enforces strict `>`, but defence-in-depth: `NegativeElapsedTime` if it ever does.
...
| 11 | `bond_terms` | Read-only. Seeds `["bond_terms", mint]`, owned by `cmtat-bond`. |
| 12 | `coupon` | Read-only. Seeds `["coupon", mint, coupon_id]`, owned by `cmtat-coupon`. |
| 13 | `holder_balance_snapshot` | Forwarded to the snapshot CPI. Seeds `["snapshot_holderbalance", mint, holder_token_account]`, owned by `cmtat-snapshot`. |
...
| 16 | `snapshot_program` | Address-pinned to `cmtat_snapshot::ID`. |
| 17 | `system_program` | — |
