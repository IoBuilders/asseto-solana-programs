...
pub fn deploy_mint(ctx: Context<DeployMint>, params: DeployMintParams) -> Result<()> {
    ...
    // (owned by cmtat-transfer-hook) and the program to call on every transfer
    // to cmtat-transfer-hook.
    invoke(
    ...
    // is Frozen (step 5). The freeze_authority PDA is owned by cmtat-freeze
    // (FREEZE_AUTHORITY_PROGRAM_ID = cmtat_freeze::ID). cmtat-freeze uses this
    ...
    cmtat_transfer_hook::cpi::initialize_extra_account_meta_list(
        CpiContext::new_with_signer(
            ...
            cmtat_transfer_hook::cpi::accounts::InitializeExtraAccountMetaList {
...
pub struct DeployMint<'info> {
    ...
    pub cmtat_transfer_hook_program: UncheckedAccount<'info>,
