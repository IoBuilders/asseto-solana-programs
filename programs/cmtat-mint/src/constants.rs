use anchor_lang::prelude::Pubkey;

/// Program ID of cmtat-deploy — the program that owns mint_owner_pda accounts.
/// Cannot use `pub use cmtat_deploy::ID` because cmtat-deploy depends on cmtat-mint
/// (for the mint authority program ID), which would create a circular dependency.
/// Must be kept in sync with cmtat-deploy's declare_id! manually.
pub const CMTAT_DEPLOY_PROGRAM_ID: Pubkey = Pubkey::new_from_array([
    0x16, 0xa1, 0xfc, 0x53, 0xc5, 0xea, 0x47, 0x05, 0xd0, 0x7a, 0xc8, 0x28, 0xe3, 0x7c, 0x4b, 0xb2,
    0x74, 0x78, 0x21, 0x82, 0xc5, 0x66, 0x70, 0xfe, 0xf4, 0x76, 0x79, 0xd5, 0x0f, 0x7f, 0x30, 0xe4,
]);

/// Sourced directly from the cmtat-freeze crate — single source of truth.
/// To update: change declare_id! in cmtat-freeze/src/lib.rs and Anchor.toml.
pub use cmtat_freeze::ID as CMTAT_FREEZE_PROGRAM_ID;

/// Sourced directly from the cmtat-snapshot crate — single source of truth.
/// To update: change declare_id! in cmtat-snapshot/src/lib.rs and Anchor.toml.
pub use cmtat_snapshot::ID as CMTAT_SNAPSHOT_PROGRAM_ID;

/// Sourced directly from the cmtat-transfer-control crate — single source of truth.
/// To update: change declare_id! in cmtat-transfer-control/src/lib.rs and Anchor.toml.
pub use cmtat_transfer_control::ID as CMTAT_TRANSFER_CONTROL_PROGRAM_ID;

/// cmtat-deactivate program ID.
/// Hardcoded — kept in sync manually with cmtat-deactivate's declare_id!.
pub const CMTAT_DEACTIVATE_PROGRAM_ID: Pubkey = Pubkey::new_from_array([
    0x74, 0xb8, 0xf2, 0xcf, 0xb6, 0x45, 0x6c, 0x59, 0x10, 0xb6, 0x46, 0x0f, 0x01, 0x2f, 0xfa, 0x5c,
    0x43, 0x7b, 0xef, 0x01, 0x5a, 0x80, 0x58, 0xeb, 0xa4, 0xad, 0x50, 0x84, 0xff, 0x1f, 0x7e, 0x55,
]);
