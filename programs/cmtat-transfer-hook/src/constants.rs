...
/// Must be kept in sync with cmtat-deploy's declare_id! manually.
...
/// Sourced directly from the cmtat-snapshot crate — single source of truth.
...
/// Sourced directly from the cmtat-transfer-control crate — single source of truth.
...
/// Hardcoded — cmtat-transfer depends on cmtat-transfer-hook for the hook's
/// program ID, so we cannot import cmtat-transfer back without a circular dep.
/// Must be kept in sync with `cmtat-transfer/src/lib.rs::declare_id!` manually.
...
/// Anchor instruction discriminator for `cmtat-transfer::verify_transfer`.
...
/// Anchor instruction discriminator for `cmtat-transfer::transfer`.
