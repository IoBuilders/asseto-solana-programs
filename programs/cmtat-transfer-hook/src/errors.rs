...
pub enum TransferHookError {
    ...
    #[msg("No previous top-level instruction: cmtat-transfer::verify_transfer must run as instruction N-1")]
    NoPreviousInstruction,

    // ── Introspection: previous instruction (must be verify_transfer) ───────
    #[msg("Previous top-level instruction's program is not cmtat-transfer")]
    PrevInstructionWrongProgram,
    #[msg("Previous top-level instruction is not cmtat-transfer::verify_transfer")]
    PrevInstructionNotVerifyTransfer,
    #[msg("Previous cmtat-transfer::verify_transfer arguments do not match the transfer being hooked (amount / source / destination / mint)")]
    PrevInstructionArgumentMismatch,

    // ── Introspection: current instruction (must be transfer or transfer_checked) ──
    #[msg("Current top-level instruction's program is neither cmtat-transfer nor token-2022")]
    CurrentInstructionUnknownProgram,
    #[msg("Current top-level instruction is neither cmtat-transfer::transfer nor token-2022::transfer_checked")]
