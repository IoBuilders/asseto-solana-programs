...
/// CMTAT compliance checks have been moved to `cmtat-transfer::verify_transfer`,
...
/// `cmtat-deploy` but is no longer baked into the metalist.
pub fn initialize_extra_account_meta_list(
    ...
    // `cmtat-transfer::verify_transfer`, so the param is intentionally unused.
    let _ = deployer;

    let metas = vec![
        // 5: cmtat-snapshot program
        ExtraAccountMeta::new_with_pubkey(&constants::CMTAT_SNAPSHOT_PROGRAM_ID, false, false)?,
        ...
        // check (verifies prior cmtat-transfer::verify_transfer + current
        // cmtat-transfer::transfer / token-2022::transfer_checked).
