...
/// 3. CPIs `cmtat-snapshot::get_holderbalance_snapshot_at(coupon.snapshot_id)`
...
pub fn pay_coupon(ctx: Context<PayCoupon>, _coupon_id: u64) -> Result<()> {
    ...
    let holder_balance: u64 = cmtat_snapshot::cpi::get_holderbalance_snapshot_at(
        CpiContext::new(
    ...
    // validated by `cmtat-coupon::create_coupon`. Each coupon thus accrues
...
pub struct PayCoupon<'info> {
    ...
    ///
    /// CHECK: Forwarded to cmtat-snapshot's get_holderbalance_snapshot_at.
