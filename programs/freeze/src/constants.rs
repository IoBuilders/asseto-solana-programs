...
/// deploy program ID.
/// Hardcoded — deploy depends on freeze (for FREEZE_AUTHORITY_PROGRAM_ID),
...
/// The mint program ID.
/// Cannot use `pub use mint::ID` because mint depends on freeze
/// (for freeze/thaw CPI calls), which would create a circular dependency.
/// Must be kept in sync with mint's declare_id! manually.
...
/// The operations program ID.
/// Cannot use `pub use operations::ID` because operations depends on freeze
/// (for freeze/thaw CPI calls), which would create a circular dependency.
/// Must be kept in sync with operations's declare_id! manually.
...
/// The transfer program ID.
/// Cannot use `pub use transfer::ID` because transfer depends on freeze
/// (for freeze/thaw CPI calls), which would create a circular dependency.
/// Must be kept in sync with transfer's declare_id! manually.
