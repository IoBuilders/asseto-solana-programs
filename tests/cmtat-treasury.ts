...
describe("cmtat-treasury", () => {
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  // All workspace programs except cmtat-deploy are cast to `any` to bypass the
  ...
  const bondProgram            = anchor.workspace.CmtatBond            as Program<any>;
  const couponProgram          = anchor.workspace.CmtatCoupon          as Program<any>;
  const treasuryProgram        = anchor.workspace.CmtatTreasury        as Program<any>;
  ...
  it("pay_coupon: transfers the computed amount, debits the treasury, and creates the coupon_paid marker", async () => {
    ...
    // (See cmtat-snapshot::get_holderbalance_snapshot_at and the matching
    // assertion in cmtat-mint.ts:579-591.)
    //
