...
describe("transfer", () => {
  ...
  it("transfer: multiple sequential post-snapshot transfers do not corrupt snapshot data", async () => {
    ...
    const senderAt2 = await getHolderBalanceSnapshotAt({ mint, holderTokenAccount: source }, { snapshotId: couponId2 });
      ...
      await transfer(
        {
          ...
          signers: [sourceOwnerKeypair, rogueKeypair],
        },
        { amount: TRANSFER_AMOUNT }
      );
      assert.fail("Expected TransferControlDenied error but instruction succeeded");
    } catch (err) {
      assert.instanceOf(err, AnchorError, "should fail as AnchorError");
      const logs = (err as AnchorError).logs ?? [];
      ...
      await transfer(
        {
          ...
          destination,
          preInstructions: [anchor.web3.ComputeBudgetProgram.setComputeUnitLimit({ units: 400_000 }), verifyIx],
          signers: [sourceOwnerKeypair, rogueKeypair, provider.wallet.payer],
        },
        { amount: TRANSFER_AMOUNT }
      );
